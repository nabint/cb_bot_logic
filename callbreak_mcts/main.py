# -----------
# Callbreak MCTS Simulation: comparing MCTS bots vs Rule-based bots.
# Players 0,2 use MCTS | Players 1,3 use rule-based bot_logic.
# -----------
...
from callbreak_mcts.callbreak_mcts_node import mcts_search
...
def _rule_bid_card_picker(**kwargs):
    return kwargs["bot_logics"][kwargs["current_idx"]].select_throw_card(
        kwargs["throw_turn"],
        kwargs["dealer"],
        kwargs["played_cards"],
    )
...
def select_bid_for_player(players, bot_logics, dealer, player_index, cumulative_scores, game_round):
    ...
    try:
        return estimate_mcts_bid(
            ...
            rule_card_picker=_rule_bid_card_picker,
        ...
        for play_turn in range(CARDS_PER_HAND):
            ...
            for throw_turn in range(NUM_PLAYERS):
                ...
                if current_idx in MCTS_PLAYERS:
                    ...
                    selected = bot_logics[current_idx].select_throw_card(
                        throw_turn, dealer, played_cards[:]
                    )
...
def run_simulation(num_games=NUM_GAMES, verbose_interval=100):
    print("=" * 60)
    print("  CALLBREAK MCTS vs RULE-BASED SIMULATION")
    print("=" * 60)
    print(f"  Games: {num_games} | Rounds/game: {NUM_ROUNDS_PER_GAME}")
    print(f"  MCTS: P0, P2 | Rule: P1, P3")
    ...
    for game_num in range(1, num_games + 1):
        ...
        mcts_total_score += scores[0] + scores[2]
        rule_total_score += scores[1] + scores[3]
